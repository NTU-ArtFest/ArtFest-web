  ...
  const topics = [
    {
      ...
      img: '/all/eco.png',
      desc: `協作團隊：學生會學術部永續執行組 <br /> 臺大學生會永續組的工作主要分為政策推行及專案活動，透過對臺大校方倡議、對學生推廣及環境教育。近期關心的核心議題有：循環餐盒推動、臺大地球日系列活動、永續生活指南、climate fresk工作坊、永續職涯訪問。`
    },
    {
      id: '6hnmhdc6ea',
      title: '性慾·癒：Epilogue — A Space for Desire',
      img: '/all/sex.png',
      desc: `協作團隊：學生會性別平等工作坊 <br /> 性工坊致力於校園內的積極倡議行動，企盼能與臺大師生共同創造正向、友善、包容的校園環境。透過公開倡議活動、監督性平政策，一步一步實現友善校園的願景。`
    }
  ];
        ...
        <main className="max-w-4xl mx-auto px-4 py-12">
          <div className="grid grid-cols-1 gap-6">
            {topics.map(topic => (
              <Link
                key={topic.id}
                href={`/polis/${topic.id}?token=null`}
                className="group block rounded overflow-hidden shadow-lg transition-transform duration-200 hover:shadow-xl hover:scale-105"
              >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 px-4">
          ...
          <div className="relative bg-white rounded-xl shadow-lg w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
            <button
              ...
              aria-label="Close"
              className="absolute top-4 right-4 p-1 text-black hover:bg-gray-200 rounded-full z-10"
            >
              <X size={20} />
            </button>

            {/* 圖片區塊：手機預設高度 h-40，平板以上 h-1/2 viewport */}
            ...
            <div className="flex-1 text-black overflow-auto px-4 pb-4">
              {([1 ,2 ,3, 4].includes(step)) ? (
                (() => {
                  const lines = tutorialTexts[step].split('\n');
                  return (
                    <>
                      {/* 只放大第一行 */}
                      <p className="text-base font-semibold mb-2">{lines[0]}</p>
                      <p className="text-base whitespace-pre-line">{lines.slice(1).join('\n')}</p>
                    </>
                  );
                })()
              ) : (
                <p className="text-base whitespace-pre-line">{tutorialTexts[step]}</p>
              )}
            </div>
